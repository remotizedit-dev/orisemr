...
async function main() {
  ...
  if (existingAdmin) {
    console.log(`Current Super Admin Account:`);
  ...
  if (!newEmail) {
    ...
    const answer = await prompt(`Enter new email (leave blank to keep '${defaultEmail}'): `);
  ...
  if (!newName) {
    ...
    const answer = await prompt(`Enter name (leave blank to keep '${defaultName}'): `);
  ...
  if (existingAdmin) {
    ...
    if (authResult?.user) {
      ...
      console.log(`- Email: ${newEmail.toLowerCase().trim()}`);
      console.log(`- Name:  ${newName.trim()}`);
    }
  }
