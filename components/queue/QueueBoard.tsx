    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 glass-panel p-5 rounded-3xl border border-[#E4E4E7] shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-white border border-[#E4E4E7] rounded-2xl shadow-xs">
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl bg-[#30D158] hover:bg-[#28b84d] text-white font-black text-xs sm:text-sm flex items-center gap-2 shadow-md hover:shadow-lg transition disabled:opacity-50 cursor-pointer"
          >
            {isCallingNext ? (
          ...
          <button
            ...
            className={`px-3 py-2 rounded-2xl text-xs font-bold flex items-center gap-1.5 border transition cursor-pointer shadow-2xs ${
          ...
          >
            <UserX className={`w-3.5 h-3.5 ${inactiveItems.length > 0 ? "text-amber-700" : "text-[#9CA3AF]"}`} />
            <span>Missed</span>
            {inactiveItems.length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full bg-amber-200 text-amber-900 font-extrabold text-[10px] font-mono">
          ...
          <div className="relative" ref={tvMenuRef}>
            <button
              type="button"
              onClick={() => setIsTvMenuOpen(!isTvMenuOpen)}
            ...
            >
              ...
              <span>TV Screen</span>
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isTvMenuOpen ? "rotate-180" : ""}`} />
            </button>

            {isTvMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white border border-[#E4E4E7] rounded-2xl shadow-xl p-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
                <Link
                  href="/app/queue/display"
                  target="_blank"
                  onClick={() => setIsTvMenuOpen(false)}
                  className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold text-[#1C1C1E] hover:bg-[#F4F4F5] transition"
                >
                  <Tv className="w-4 h-4 text-[#2A5CAA] shrink-0" />
                  <div className="flex flex-col">
                    <span>Open TV Display</span>
                    <span className="text-[10px] font-normal text-[#6B7280]">Full screen for waiting room</span>
                  </div>
                </Link>
          ...
          {/* Workflow Guide Toggle - Small & Compact */}
          <button
            type="button"
            onClick={() => setShowGuide(!showGuide)}
            className="p-2 rounded-xl bg-white border border-[#E4E4E7] text-[#6B7280] hover:text-[#1C1C1E] hover:bg-[#F4F4F5] transition cursor-pointer shadow-2xs"
            title="How Queue Works"
          >
            <HelpCircle className="w-4 h-4 text-[#2A5CAA]" />
          </button>
        </div>
      </div>
